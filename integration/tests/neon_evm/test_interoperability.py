...
from integration.tests.neon_evm.solana_utils import write_transaction_to_holder_account, \
    EvmLoader, execute_transaction_steps_from_account_with_solana_call
...
from integration.tests.neon_evm.utils.layouts import FINALIZED_STORAGE_ACCOUNT_INFO_LAYOUT
...
class TestInteroperability:
    ...
    def test_execute_compute_budget(self, sender_with_tokens, solana_caller,
        ...
        resp = execute_transaction_steps_from_account_with_solana_call(operator_keypair, evm_loader, treasury_pool,
                                                                       holder_acc,
    ...
    def test_execute_call_memo(self, sender_with_tokens, neon_api_client, holder_acc, second_operator_keypair,
                     evm_loader, treasury_pool, sol_client):
        holder_acc = create_holder(second_operator_keypair)
        contract = deploy_contract(second_operator_keypair, sender_with_tokens, "call_solana_test",
                        evm_loader, treasury_pool, contract_name='Test')

        signed_tx = make_contract_call_trx(sender_with_tokens, contract, "call_memo()")
        write_transaction_to_holder_account(signed_tx, holder_acc, second_operator_keypair)
        ...
        resp = execute_transaction_steps_from_account_with_solana_call(second_operator_keypair, evm_loader, treasury_pool,
                                                                       holder_acc,
